...
def check_env_vars(logger: Optional[logging.Logger] = None) -> None:
    ...
    Note: ANTHROPIC_API_KEY is now optional - will use Claude CLI subscription if not provided.
    """
    required_vars = [
        "CLAUDE_CODE_PATH",
    ]
    ...
    # Log info about API key usage
    if os.getenv("ANTHROPIC_API_KEY"):
        if logger:
            logger.info("Using ANTHROPIC_API_KEY from environment")
    else:
        if logger:
            logger.info("No ANTHROPIC_API_KEY found - will use Claude CLI subscription")
